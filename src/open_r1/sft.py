...
#         dataset.extend(dataset_subsc)
        
#     with open('./load_data/dataset/foundation_new_telco_common_202504_train.json', 'r', encoding='utf-8') as f:
#         dataset_telco = json.load(f)
#         random.shuffle(dataset_telco)
#         dataset_telco = dataset_telco[:num_samples]
#         dataset.extend(dataset_telco)
        
#     with open('./load_data/dataset/foundation_candidate_sampled_fee_202504_train.json', 'r', encoding='utf-8') as f: # foundation_fee_202504_train
#         dataset_fee = json.load(f)
#         random.shuffle(dataset_fee)
#         dataset_fee = dataset_fee[:num_samples]
#         dataset.extend(dataset_fee)
        
#     with open('./load_data/dataset/foundation_new_meta_explanation_202504_train.json', 'r', encoding='utf-8') as f:
#         dataset_meta = json.load(f)
#         # dataset_meta = [dataset_meta[str(i)] for i in range(len(dataset_meta))]
#         dataset.extend(dataset_meta)
...
#     # task1: meta
#     with open(f"./src/open_r1/sasrec/skt_dataset/foundation_{domain}_202506_train.json", 'r', encoding='utf-8') as f:
...
#     with open(f"./src/open_r1/sasrec/skt_dataset/foundation_{domain}_202506_test.json", 'r', encoding='utf-8') as f:
...
#     # gudok case
#     def make_sft_conversation(example, add_generation_prompt = False):
#         prompt = []

#         prompt.append({"role": "system", "content": example["system_prompt"]})        
#         prompt.append({"role": "user", "content": example["instruction"] + "\n\n" + example["user_context"] + "\n\n" + example["candidate"] })
#         prompt.append({"role": "assistant", "content": example["output"] + tokenizer.eos_token})
#         return {'text':tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=add_generation_prompt)}
